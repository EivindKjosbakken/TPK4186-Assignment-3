...
class GUI:
    ...
    def createGUI(self, robots : list, canvas : Canvas, zones : list, warehouse, rootWindow, displayWarehouse : bool, shouldPrint : bool, warehouseStats : WarehouseStats, catalog, maxTimeStep, timeForReload, truckloadTime, customerOrderTimes, truckloadSizePer5000Time, customerOrderSizePer5000Time):
        ...
        button2.pack()
        button3 = Button(frame, text = "Go to 100th next timestep", command=partial(self.next100TimeStepsTkinter, warehouse, robots, canvas, zones, shouldPrint, warehouseStats, catalog, maxTimeStep, timeForReload, truckloadTime, customerOrderTimes, truckloadSizePer5000Time, customerOrderSizePer5000Time)) 
        button3.pack()
    ...
    def nextTimeStepTkinter(self, warehouse, robots : list, canvas : Canvas, zones : list, shouldPrint : bool, warehouseStats : WarehouseStats, catalog, maxTimeStep, timeForReload, truckloadTime, customerOrderTimes, truckloadSizePer5000Time, customerOrderSizePer5000Time):
        ...
        self.simulator.addTruckloadsAndCustomerOrders(warehouse, catalog, shouldPrint, maxTimeStep, timeForReload, truckloadTime, customerOrderTimes, truckloadSizePer5000Time, customerOrderSizePer5000Time)
        ...
        if (True):
            ...
            #for cell in warehouse.getAllStorageCells():
            #    p.printCell(cell)
            #print("CURRENT")
            ##p.printTruckload(warehouse.getCurrentTruckload())
            #p.printCustomerOrder(warehouse.getCurrentCustomerOrder())
    ...
    def next100TimeStepsTkinter(self, warehouse, robots : list, canvas : Canvas, zones : list, shouldPrint : bool, warehouseStats : WarehouseStats, catalog, maxTimeStep, timeForReload, truckloadTime, customerOrderTimes, truckloadSizePer5000Time, customerOrderSizePer5000Time):
        """for the button in the tkinter application"""
        for i in range(100):
            self.simulator.addTruckloadsAndCustomerOrders(warehouse, catalog, shouldPrint, maxTimeStep, timeForReload, truckloadTime, customerOrderTimes, truckloadSizePer5000Time, customerOrderSizePer5000Time)
            print(f"\n ____TIMESTEP {self.simulator.getTimeStep()}____")
